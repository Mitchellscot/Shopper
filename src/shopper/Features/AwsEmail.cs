...
namespace shopper.Features
{
    public class AwsEmail
    {
        ...
        public AwsEmail(CsvStorage data, List<Product> products, string settingsFilePath)
        {
            _data = data;
        ...
        {
            (string from, string to) = (_data.GetSettingsFromFile(_settingsFilePath).Result.FromAddress, _data.GetSettingsFromFile(_settingsFilePath).Result.ToAddress);
